...
from models import *
from dataset_elmo import *

parser = argparse.ArgumentParser(description='Experiment 1 Elmo Embeddings')
...
lstm = LSTM1(embedding_length=1024, hidden_size=300).to(device)
...
def train_lstm1(currepoch, epoch):
    ...
    for batch_idx in range(len(dataloader)):
        ...
        with open("./logs/elmolstm1_train_loss.log", "a+") as lfile:
            lfile.write("{}\n".format(train_loss / total))

        with open("./logs/elmolstm1_train_acc.log", "a+") as afile:
        ...
        torch.save(lstm.state_dict(), './weights/networkelmolstm1_train.ckpt')
        with open("./information/elmolstm1_info.txt", "w+") as f:
    ...
    if(currepoch + 1 % 10 == 0):
        torch.save(lstm.state_dict(), './checkpoints/networkelmolstm1_train_epoch_{}.ckpt'.format(currepoch + 1))
...
def test_lstm1(currepoch, epoch):
    ...
    for batch_idx in range(len(dataloader)):
        ...
        with open("./logs/elmolstm1_test_loss.log", "a+") as lfile:
            lfile.write("{}\n".format(test_loss / total))

        with open("./logs/elmolstm1_test_acc.log", "a+") as afile:
            afile.write("{}\n".format(correct / total))
